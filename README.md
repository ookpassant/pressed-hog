...
- The **personal API key** (`phx_…`) for the dashboard is stored in `wp_options`, used only server-side, and never printed on the front end. Create it with the read-only *Query* scope.
- All admin AJAX endpoints require the `manage_options` capability and a nonce.
- The reverse proxy only relays to your configured PostHog host — the target host is fixed server-side, so it cannot be used as an open proxy to arbitrary destinations.
